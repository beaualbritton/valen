<script lang="ts">
let {toggle, handleToggle = $bindable()} : {toggle:boolean, handleToggle(): void}= $props()
...
async function submit(e: Event)
{
  ...
  const creationResponse = await fetch(`/settings/ssh/create`, {
    method: "POST",
    ...
    <div class="bg-surface rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      <form method="post" onsubmit={submit} class="p-6 md:p-8">
        <div class="flex items-center justify-between mb-8">
          <h2 class = "text-3xl font-bold px-5">add ssh key</h2>
        ...
        <div class="w-full flex justify-center">
          <div class="w-xl space-y-4">
            <div class="flex items-center gap-4">
              <input bind:value={public_key} type="text" placeholder="your .pub key contents" required class="flex-1 px-4 py-3 border-2 rounded-xl bg-surface brightness-95 text-main focus:outline-none active:outline-none focus:ring-2 focus:ring-[#a7c080] shadow-sm resize-none" />
            </div>
          </div>
        </div>
