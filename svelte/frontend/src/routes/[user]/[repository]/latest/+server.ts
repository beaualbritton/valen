...
export const GET: RequestHandler = async ({ params, url }) => 
{
  ...
  const objectId = url.searchParams.get('oid');
  const fromCommitOID = url.searchParams.get('fromCommitOID')

  //Important, calling on server
  const response = await fetchLatestCommitForObject(user, repository, objectId, fromCommitOID);
